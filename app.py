...
def init_db():
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS beneficiarios (
            id INTEGER PRIMARY KEY AUTOINCREMENT, nombre TEXT, fecha_nacimiento TEXT, seccion TEXT,
...
async def ver_registros(request: Request):
    ...
    registros = conn.execute("SELECT * FROM beneficiarios ORDER BY id DESC").fetchall()
    conn.close()
...
async def eliminar_registro(reg_id: int, password: str = Form(...)):
    ...
    conn = sqlite3.connect(DB_FILE)
    conn.execute("DELETE FROM beneficiarios WHERE id = ?", (reg_id,))
    conn.commit()
    conn.close()
...
async def editar_registro(
    ...
    conn.execute('''
        ...
        SET nombre=?, fecha_nacimiento=?, seccion=?, calle=?, num_ext=?, num_int=?, cp=?, fraccionamiento=?, telefono=?, folio_tarjeta=? 
        WHERE id=?
